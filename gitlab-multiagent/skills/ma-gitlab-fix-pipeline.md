ma-gitlab-fix-pipeline

Fetch pipeline logs, diagnose the failure, fix the Terraform code that caused the error, and raise or update the MR. Never modify pipeline files — only fix `terraform/` and `env/` files.
